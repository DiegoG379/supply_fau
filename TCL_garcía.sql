...
DELIMITER //
CREATE FUNCTION GetRetiredMaterialsInfoByWorker(workerId INT) RETURNS VARCHAR(4000)
DETERMINISTIC
BEGIN
...
SELECT GetRetiredMaterialsInfoByWorker(7);
...
-- Triggers tabla entry

-- Este trigger verifica que la cantidad ingresada en la tabla entry no sea negativa, generando un error si lo es.
DELIMITER //
CREATE TRIGGER before_insert_check_quantity
BEFORE INSERT ON entry
FOR EACH ROW
BEGIN
    IF NEW.entered_quantity < 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'La cantidad ingresada no puede ser negativa';
    END IF;
END; //
...
-- Triggers tabla outbound

-- Este trigger verificará que el material retirado exista en la tabla materials.
DELIMITER //
CREATE TRIGGER before_insert_check_material
BEFORE INSERT ON outbound
FOR EACH ROW
BEGIN
    DECLARE material_count INT;
    SELECT COUNT(*) INTO material_count FROM materials WHERE material_id = NEW.material_id;
    IF material_count = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'El material a retirar no existe en la tabla materials';
    END IF;
END; //
...
-- Este trigger verifica la disponibilidad de un material antes de permitir la inserción de un registro en la tabla maintenance_log
DELIMITER //
CREATE TRIGGER check_material_availability
BEFORE INSERT ON maintenance_log
FOR EACH ROW
BEGIN
    IF (SELECT stock FROM materials WHERE material_id = NEW.material_id) <= 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'El material no está disponible para mantenimiento';
    END IF;
END; //


-- El trigger automáticamente registra la fecha en la columna "maintenance_date" al insertar una nueva entrada en la tabla maintenance_log
DELIMITER //
CREATE TRIGGER maintenance_log_insert_trigger
BEFORE INSERT ON maintenance_log
FOR EACH ROW
BEGIN
    SET NEW.maintenance_date = NOW();
END; //
...
DROP USER IF EXISTS juan_martinez@localhost;
CREATE USER juan_martinez@localhost IDENTIFIED BY 'Abastecimineto_2022';

GRANT SELECT ON supply_fau.* TO juan_martinez@localhost;  -- Asignar permisos de solo lectura a todas las tablas
REVOKE DELETE ON supply_fau.* FROM juan_martinez@localhost;  -- Evitar la posibilidad de eliminar registros de cualquier tabla


-- Crear usuario con permisos de lectura, inserción y modificación
DROP USER IF EXISTS maria_lopez@localhost;
CREATE USER maria_lopez@localhost IDENTIFIED BY 'Maria_LOPEZ379';

GRANT SELECT ON supply_fau.* TO maria_lopez@localhost;  -- Asignar permisos de lectura a todas las tablas.
GRANT INSERT ON supply_fau.* TO maria_lopez@localhost;  -- Asignar permisos de inserción a todas las tablas.
GRANT UPDATE ON supply_fau.* TO maria_lopez@localhost;  -- Asignar permisos de modificación a todas las tablas.
REVOKE DELETE ON supply_fau.* FROM maria_lopez@localhost;  -- Evitar la posibilidad de eliminar registros de cualquier.
...
-- Transacción para eliminar 3 tareas de mantenimiento de la tabla maintenance_log.
START TRANSACTION;

DELETE FROM maintenance_log WHERE log_id = 3;
DELETE FROM maintenance_log WHERE log_id = 7;
DELETE FROM maintenance_log WHERE log_id = 9;

-- ROLLBACK; -- Hacer un rollback para revertir los cambios.
-- COMMIT; -- Realiza un commit para confirmar los cambios.
-- SELECT * FROM maintenance_log; -- Consultar la tabla maintenance_log para verificar cambios.


-- La transacción inserta 8 registros en materials con savepoints después de los registros #4 y #8.
START TRANSACTION;

INSERT INTO materials (part_number, material_name, serial_number, stock) VALUES -- Inserta cuatro registros aleatorios en la tabla materials
    ('LN65019-05060', 'BOLT', '', 75),
    ('LN65019-05095', 'BOLT', '', 146),
    ('212-41207-9', 'BUSHING', '', 89),
    ('LN9484M5', 'NUT', '', 159);
SAVEPOINT savepoint_one; -- Agrega un savepoint después de la inserción del registro #4

INSERT INTO materials (part_number, material_name, serial_number, stock) VALUES -- Inserta otros cuatro registros aleatorios
    ('LN9484M9', 'NUT', '', 190),
    ('MS29513-113', 'PACKING', '', 50),
    ('MS29513-237', 'PACKING', '', 40),
    ('LN9439M5X02', 'SCREW', '', 65);
SAVEPOINT savepoint_two; -- Agrega un savepoint después de la inserción del registro #8

-- ROLLBACK TO savepoint_one; -- Hacer un rollback para revertir los cambios hasta el savepoint_one.
-- ROLLBACK; -- Hacer un rollback para revertir los cambios.
-- COMMIT; -- Realiza un commit para confirmar los cambios.
-- SELECT * FROM materials; -- Consultar la tabla materials para verificar cambios.
